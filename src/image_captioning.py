...
Choosing to use more data would result in improved captioning quality but slow training time.
"""
...
Next, use InceptionV3 (which is pretrained on Imagenet) to classify each image and 
extract features from the last convolutional layer.

First, need to convert the images into InceptionV3's expected format by:

Resizing the image to 299px by 299px
Preprocess the images using the preprocess_input method to normalize the image so 
that it contains pixels in the range of -1 to 1, which matches the format of the 
images used to train InceptionV3.
"""
...
Now, create a tf.keras model where the output layer is the last convolutional layer in the 
InceptionV3 architecture. The shape of the output of this layer is 8x8x2048. Then, use the last 
convolutional layer because attention is being used. Don't perform this initialization during 
training because it could become a bottleneck.
...
Pre-process each image with InceptionV3 and cache the output to disk. Caching the output in RAM would be faster but 
also memory intensive, requiring 8 * 8 * 2048 floats per image.

Performance could be improved with a more sophisticated caching strategy (for example, by sharding the images to 
reduce random access disk I/O), but that would require more code.
"""
...
"""
# Change these parameters according to your system's configuration
BATCH_SIZE = 64
BUFFER_SIZE = 1000
embedding_dim = 256
units = 512
